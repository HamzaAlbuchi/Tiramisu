...
public class DebateService {
        ...
        for (int i = 0; i < n; i++) {
            if (i % 2 == 0) {
                exchanges.add(new DebateExchange(
                        "A", DEFAULT_MODEL_A, "Pro", TEMP_A, buildArgumentA(topic, i / 2, style)));
            } else {
                exchanges.add(new DebateExchange(
                        "B", DEFAULT_MODEL_B, "Against", TEMP_B, buildArgumentB(topic, i / 2, style)));
            }
        }
    ...
    private String buildArgumentA(String topic, int round, String style) {
        String prefix = stylePrefix(style);
        ...
        String body;
        if (idx == 1) {
            body = String.format(Locale.US, frames[idx], round + 1, topic);
        } else {
            body = String.format(Locale.US, frames[idx], topic);
        }
        return prefix + body;
    }

    private String buildArgumentB(String topic, int round, String style) {
        String prefix = stylePrefix(style);
        String[] frames = new String[]{
    ...
    private static String stylePrefix(String style) {
        if ("casual".equals(style)) {
            return "[Casual] ";
        }
